"use client";

import Link from "next/link";
import { useLangue } from "@/components/LangueProvider";

export default function Home() {
  const { t } = useLangue();
  const audiences = [
    { emoji: "👨‍👩‍👧", titre: t.audienceParentsTitre, texte: t.audienceParents },
    { emoji: "🧑🏾‍🏫", titre: t.audienceFacilitateursTitre, texte: t.audienceFacilitateurs },
    { emoji: "🏛️", titre: t.audienceInstitutionTitre, texte: t.audienceInstitution },
  ];
  return (
    <div className="flex flex-col gap-10">
      <section className="rounded-3xl bg-surface p-8 shadow-sm sm:p-12">
        <p className="mb-2 text-sm font-bold uppercase tracking-wide text-accent">
          {t.brand} · {t.tagline}
        </p>
        <h1 className="max-w-2xl text-3xl font-extrabold leading-tight text-ink sm:text-4xl">
          {t.sousTitre}
        </h1>
        <div className="mt-6 flex flex-wrap gap-3">
          <Link
            href="/parent"
            className="rounded-full bg-brand px-6 py-3 text-base font-bold text-white transition-colors hover:bg-brand-dark"
          >
            {t.ctaParent}
          </Link>
          <Link
            href="/apprendre"
            className="rounded-full border-2 border-brand px-6 py-3 text-base font-bold text-brand-dark transition-colors hover:bg-brand/10"
          >
            {t.ctaApprendre}
          </Link>
        </div>
      </section>

      <section className="grid gap-4 sm:grid-cols-3">
        {audiences.map((a) => (
          <div key={a.titre} className="rounded-2xl bg-surface p-6 shadow-sm">
            <div className="mb-3 text-4xl" aria-hidden>
              {a.emoji}
            </div>
            <h2 className="mb-1 text-lg font-bold text-ink">{a.titre}</h2>
            <p className="text-sm leading-relaxed text-muted">{a.texte}</p>
          </div>
        ))}
      </section>
    </div>
  );
}
